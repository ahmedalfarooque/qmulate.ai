...
const LAYERS = [
  { num:"01", name:"Governance Layer", color:"#00D4FF", desc:"Constitutional documents, entity structures, voting rights, and succession protocols. The permanent framework that governs everything below it." },
...
export default function About() {
  return (
    <main>
            ...
            {[
              ...
              <motion.div key={item.label} {...FU(i*.1)}>
                <GlassCard style={{padding:"clamp(32px,4vw,52px)",height:"100%"}}>
                  ...
                  <h3 className="t-h3" style={{color:"#F1F5F9",marginBottom:16,lineHeight:1.3}}>{item.title}</h3>
                  <p className="t-md" style={{color:"#64748B",lineHeight:1.85}}>{item.body}</p>
                </GlassCard>
              </motion.div>
            ))}
          </div>
      ...
      {/* ── Founding Principle ── */}
      <section style={{padding:"clamp(60px,8vw,100px) 0",position:"relative",overflow:"hidden",background:"rgba(6,11,20,.7)"}}>
        ...
        <div className="container" style={{position:"relative",zIndex:1,textAlign:"center"}}>
          ...
          <motion.blockquote {...FU(.08)} style={{fontSize:"clamp(24px,3.5vw,52px)",fontWeight:800,color:"#F1F5F9",lineHeight:1.2,maxWidth:900,margin:"0 auto 32px",letterSpacing:"-0.035em"}}>
            "Wealth, structured to <span className="gt-c">outlast its makers.</span>"
          </motion.blockquote>
          <motion.p {...FU(.16)} className="t-xs" style={{color:"#334155"}}>QMULATE · FOUNDED 2019 · RIYADH, SAUDI ARABIA</motion.p>
        </div>
      </section>
                ...
                <motion.div key={p.n} {...FU(.06+i*.09)}>
                  <GlassCard style={{padding:"24px 28px"}}>
                    <div style={{display:"flex",gap:16,alignItems:"flex-start"}}>
                      <div style={{width:34,height:34,borderRadius:10,background:"rgba(0,212,255,.08)",border:"1px solid rgba(0,212,255,.2)",display:"flex",alignItems:"center",justifyContent:"center",fontFamily:"var(--font-geist-mono,'Courier New'),monospace",fontSize:11,color:"#00D4FF",fontWeight:700,flexShrink:0}}>{p.n}</div>
                      <div>
                        <div style={{fontSize:14,fontWeight:700,color:"#F1F5F9",marginBottom:6}}>{p.title}</div>
                    ...
                    <div style={{display:"flex",gap:20,alignItems:"flex-start"}}>
                      <div>
                        <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:10}}>
                          ...
                          <h3 style={{fontSize:16,fontWeight:700,color:"#F1F5F9"}}>{layer.name}</h3>
                          <div style={{width:6,height:6,borderRadius:"50%",background:layer.color,boxShadow:`0 0 10px ${layer.color}`,marginLeft:"auto"}}/>
                        </div>
                        <p className="t-sm" style={{color:"#64748B",lineHeight:1.75}}>{layer.desc}</p>
                      </div>
                    </div>
                  </GlassCard>
                  {/* Connector dot */}
            ...
            {TEAM.map((member,i)=>(
              <motion.div key={member.name} {...FU(i*.08)}>
                <GlassCard style={{padding:"clamp(24px,3vw,36px)",textAlign:"center"}}>
                  ...
                  <div style={{fontSize:15,fontWeight:700,color:"#F1F5F9",marginBottom:4}}>{member.name}</div>
                  <div style={{fontSize:13,color:"#00D4FF",marginBottom:8}}>{member.role}</div>
                  <div className="t-xs" style={{color:"#334155",textTransform:"none",letterSpacing:0,fontSize:12}}>{member.area}</div>
                </GlassCard>
              </motion.div>
            ))}
          </div>
      ...
      <section className="section-sm" style={{textAlign:"center",position:"relative",overflow:"hidden"}}>
        ...
        <div className="container" style={{position:"relative",zIndex:1}}>
          ...
          <motion.p {...FU(.08)} className="t-lg" style={{color:"#64748B",marginBottom:36}}>All introductions are private and confidential.</motion.p>
