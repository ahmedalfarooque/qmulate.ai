...
const SOLUTIONS = [
  {icon:"🏛️",title:"Governance Platform",color:"#00D4FF",desc:"End-to-end governance infrastructure — constitutional documents, mandate frameworks, entity structures, and succession protocols — deployed as a unified, auditable system.",features:["Constitutional document library","Board governance tools","Succession protocol engine","Mandate tracking dashboard","Audit trail and documentation","Regulatory compliance monitoring"]},
    ...
    <main>
            ...
            {SOLUTIONS.map((s,i)=>(
              <motion.div key={s.title} {...FU(i*.07)}>
                <GlassCard style={{padding:"clamp(24px,3vw,40px)",height:"100%"}}>
                  ...
                  <h3 style={{fontSize:18,fontWeight:700,color:"#F1F5F9",marginBottom:10,lineHeight:1.3}}>{s.title}</h3>
                  <p className="t-sm" style={{color:"#64748B",lineHeight:1.75,marginBottom:24}}>{s.desc}</p>
        ...
        <div className="container" style={{position:"relative",zIndex:1}}>
          ...
          <div style={{maxWidth:900,margin:"0 auto"}}>
            {[
              ...
              {label:"Asset & Entity Foundation",color:"#00D4FF",sub:"All properties · Entities · Holdings · Structures"},
            ].map((layer,i)=>(
              <motion.div key={layer.label} {...FU(.08+i*.1)} style={{marginBottom:8}}>
                <div style={{
                  ...
                  <div style={{fontWeight:700,color:"#F1F5F9",fontSize:"clamp(13px,1.5vw,16px)"}}>{layer.label}</div>
                  <div style={{fontSize:12,color:"#64748B",textAlign:"right",maxWidth:360}}>{layer.sub}</div>
      ...
      <section className="section-sm" style={{textAlign:"center",position:"relative",overflow:"hidden"}}>
        ...
        <div className="container" style={{position:"relative",zIndex:1}}>
          ...
          <motion.p {...FU(.08)} className="t-lg" style={{color:"#64748B",marginBottom:36}}>We offer a private demonstration for qualified principals.</motion.p>
