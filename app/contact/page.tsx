...
export default function Contact() {
  ...
  const inputStyle = (field:string):React.CSSProperties => ({
    ...
    borderRadius:12,padding:"13px 16px",fontSize:14,color:"#F1F5F9",
              ...
              <motion.div {...FU(.12)} style={{display:"flex",flexDirection:"column",gap:24,marginTop:36}}>
                {[
                  ...
                  <div key={item.label} style={{display:"flex",gap:16,alignItems:"flex-start"}}>
                    ...
                    <div>
                      <div className="t-xs" style={{color:"#334155",marginBottom:4}}>{item.label}</div>
                      <div style={{fontSize:14,color:"#94A3B8"}}>{item.val}</div>
                    </div>
                  </div>
                ))}
              </motion.div>
              ...
              <motion.div {...FU(.2)} style={{marginTop:40}}>
                <GlassCard style={{padding:"24px 28px"}}>
                  <div style={{fontSize:13,color:"#64748B",lineHeight:1.8}}>
                    <strong style={{color:"#F1F5F9",display:"block",marginBottom:8}}>Minimum Mandate</strong>
                ...
                {sent ? (
                  <motion.div key="success" initial={{opacity:0,scale:.95}} animate={{opacity:1,scale:1}} exit={{opacity:0}}>
                    <GlassCard style={{padding:"clamp(40px,5vw,64px)",textAlign:"center"}}>
                      ...
                      <h3 className="t-h3" style={{color:"#F1F5F9",marginBottom:12}}>Introduction received.</h3>
                      <p style={{color:"#64748B",lineHeight:1.8,fontSize:15}}>Thank you. We have received your enquiry and will respond within one business day. All communications are treated with complete discretion.</p>
                    </GlassCard>
                  </motion.div>
                ) : (
                  <motion.div key="form" initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}}>
                    <GlassCard style={{padding:"clamp(32px,4vw,52px)"}}>
                      <div className="t-xs" style={{color:"#334155",marginBottom:28}}>INTRODUCTION REQUEST</div>
                      <form onSubmit={handleSubmit} style={{display:"flex",flexDirection:"column",gap:18}}>
                        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16}}>
                          <div>
                            <label style={{fontSize:12,color:"#64748B",display:"block",marginBottom:7}}>Full name *</label>
                            <input required value={form.name} onChange={e=>setForm({...form,name:e.target.value})}
                              placeholder="Your name" style={inputStyle("name")}
                              onFocus={()=>setFocus("name")} onBlur={()=>setFocus(null)}/>
                          </div>
                          <div>
                            <label style={{fontSize:12,color:"#64748B",display:"block",marginBottom:7}}>Email address *</label>
                        ...
                        <div>
                          <label style={{fontSize:12,color:"#64748B",display:"block",marginBottom:7}}>Family / Entity name</label>
                        ...
                        <div>
                          <label style={{fontSize:12,color:"#64748B",display:"block",marginBottom:7}}>Area of interest</label>
                          <select value={form.reason} onChange={e=>setForm({...form,reason:e.target.value})}
                            ...
                            {REASONS.map(r=><option key={r} value={r} style={{background:"#060B14"}}>{r}</option>)}
                          </select>
                        </div>
                        <div>
                          <label style={{fontSize:12,color:"#64748B",display:"block",marginBottom:7}}>Message *</label>
                        ...
                        <p style={{fontSize:11,color:"#334155",textAlign:"center"}}>All introductions are private and confidential. We do not share your information.</p>
                      </form>
                    </GlassCard>
                  </motion.div>
                )}
